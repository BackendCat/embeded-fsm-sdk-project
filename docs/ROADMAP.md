...
- **D1 — the static EdgeForge-modeled docs/landing site** (the slipped v1.5 item) + the OSS-files reconcile (LICENSE/CONTRIBUTING/CoC presence) + the doc-honesty staleness pass (README/Doc-07/Doc-14 — Doc 31 §6 H6) that must precede/accompany D1 so it does not broadcast a stale README.
- **The VS Code extension FE-hygiene batch** — flat ESLint-9 + `typescript-eslint@8` (C1-F3), `madge` import-cycle guard (C1-F7), the `noUncheckedIndexedAccess` strictness delta (one coherent tooling pass, deliberately *not* churned onto the v1.5 Marketplace pass).
- **The WS-server-or-not decision** — the Doc 13 WS *simulator* server is its own later concern (with its own SEC-P0-1 gate); whether it is the v1.6 epic or deferred again is an owner/orchestrator decision, **not** pre-resolved here.
- **The parked Certifiability / MISRA–CERT-C owner-review** (Doc 31 §3/§4 OWNER-C) — a documented MISRA-C/CERT-C compliance posture for the generated C; a named parked ROADMAP-v1.6+ candidate requiring an explicit owner GO to schedule (composes with verification into a "Certifiability & factory-integration" arc; *not* auto-started).
...
Per the owner's GLOBAL quality-first + stage-gate roadmap (2026-05-17): **factory-reliability/CI epic (this quality milestone — SHIPPED)** → **v1.6 = the landing-page + the doc-honesty pass** (D1 the static EdgeForge-modeled docs/landing site, slipped from v1.5 not cut; **+** the README/Doc-07/Doc-14 doc-honesty staleness pass that must precede/accompany D1 so the public site is accurate — Doc 31 §6 H6 / `ASSESSMENT_PRODUCT_AND_DOCS_2026_05_16.md` PART B; the OSS-files reconcile LICENSE/CONTRIBUTING/CoC) → **the visual DSL constructor = the FINAL capstone feature** (the owner-named end-state — a visual statechart constructor on top of the now-factory-trustable, fully-tested pipeline). The parked Certifiability/MISRA–CERT-C *codegen-mode* arc (OWNER-3) remains a named candidate requiring an explicit owner GO (this epic delivered only the *testing/proof* half of Certifiability). These are **recorded, not auto-scheduled** — the orchestrator/owner owns the sequencing call (the §3 / Doc 31 §3 "surface, don't auto-scope" discipline).
