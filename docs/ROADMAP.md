...
- **Submachine support — FULL EPIC (status corrected 2026-05-15).** Prior ROADMAP/backlog text claimed "IR + analyzer already lower submachine references; codegen emits no-op." **That was false** — verified absent end-to-end (no grammar production for `submachine`/`state … is X`; orphan `KwIs`/`KwSubmachine` tokens consumed by no rule; `lower.rs` hard-codes `submachines: Vec::new()`; IR `SubmachineRef` only built by a unit-test fixture; codegen/simulator arms are defensive-only for a never-produced variant). This was aspirational prose never reconciled against code — the P0-1 pattern. Submachine is therefore a multi-wave epic, not a codegen wave: **W2a** parser+CST+AST (`submachine_decl`, `state … is X`) → **W2b** analyzer symbol-table + `lower.rs` → `StateNode::Submachine` + populate `MachineObject.submachines` + real FSM-E0610/E0500/E0501 → **W2c** simulator submachine RTC semantics (Doc 08 §12) → **W2d** codegen (instance-per-reference, delegate dispatch, Final→completion) + behavioural example/tests. v1.0.0's "full UML" wording was overstated by exactly this gap (the 3 shipped examples don't use submachines, so the gate verification was accurate for what it tested).
