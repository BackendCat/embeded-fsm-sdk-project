...
A completion transition has `trigger = null`. It is selected if and only if the source
state's completion event is enqueued (per §9.1 below) and no event-triggered
transition is enabled. **Completion transitions MAY carry a guard**;
standard transition-selection rules apply (per Doc 00 §B-07 / §I-22).
The `FSM-E0301` code that previously forbade this is retired (Doc 10
deprecation policy).
...
**except** when computing the exit/entry set of an EXTERNAL self-transition
(`source == target` and `kind == external`), in which case the effective
LCA is `source.parent`. See Doc 04 §8.1 / §8.3 for the syntactic distinction
between external (`->`) and local (`~>`) transitions. The full exit/entry
table by transition kind:
...
Per Doc 00 §B-09 / §I-21. The transition kind is carried on
`TransitionObject.kind` in the IR (Doc 09 §6).
...
If no history is stored and a `-> History` transition is taken, the machine
enters the history pseudo-state's `defaultTarget`. **The default target is
MANDATORY** per Doc 00 §B-14: absence is `FSM-E0111` at compile time, so the
"no default" branch is statically unreachable. The previous
implementation-defined fallback no longer exists.
...
A completion event is generated by the following normative algorithm (per
Doc 00 §B-08 / §I-22). The runtime invokes this after every entry sequence
on the just-entered leaf states.

```
function maybe_enqueue_completion(state S):
    if S is a simple state:
        enqueue_completion_event(S)            # auto-fire on entry
        ...
        if for every region R of S: R.active_leaf is final:
            enqueue_completion_event(S)        # all-regions-done rule
    # else: nothing to do
```

The all-regions-done rule on Parallel states preserves the standard UML
2.5.1 §14.2.3.4.5 contract that a Parallel composite finishes only when
every region has reached a FinalState. Per-region finals on their own do
NOT fire the parent's completion. See Doc 02 §5.6 for the user-facing
table of `done` semantics by source state kind.
