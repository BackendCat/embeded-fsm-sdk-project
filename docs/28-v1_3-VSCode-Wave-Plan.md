...
> **⚠ v1.3-V1 post-implementation reconciliation (Findings 1+2 + N-2 — see Doc 00 §11.52/§11.53; full correction text: `docs/AUDIT_PHASE_V1_2026_05_16.md` §3, the durable correction-of-record).** The §11.3 post-V1 audit independently re-derived two corrections from the locked `vscode-languageclient@9.0.1` source (the R-14/R-15 stale-framing discipline of this very ledger, extended to a third finding-pair): **(1) R-4 / §3-V1(c) / §5-V1(c) — "the negotiated `positionEncoding` is UTF-8" is FALSE.** UTF-8 is **unreachable** through the official client (it hardcodes `['utf-16']` and throws on non-UTF-16); the correct negotiated value is **`"utf-16"`** and the shipped acceptance test already asserts that (the implementer **correctly diverged** from clause (c)'s literal "UTF-8" instruction — N-2; recorded so a future "test ≠ brief" mechanical check does not mis-flag the *desired* state). R-4's verified server-side negotiation code is unchanged and exact; only its *client-reachability assumption* was false — the correct surviving constraint is "do not override `clientOptions`/`initializationOptions` to suppress the client's default capabilities", not "do not strip `general.positionEncodings`". **(2) Doc 27 §2.2's `TransportKind.stdio`** is a wrong literal — the only round-tripping form is to omit `transport` on the `Executable` (Doc 00 §11.52). **The shipped V1 code is correct on both seams; these are pure documentation-of-record fixes — no code change.** A wave reading R-4 / Doc 27 §2.3 / the §3-V1(c)/§5-V1(c) acceptance clauses must read them through this row (and §11.53), exactly as R-14/R-15 mandate for the DRIFT-2/oracle prose.
