...
> ⚠ **reconciliation note (v1.4 closeout — the LOAD-BEARING correction; carry-list item 2 / W2-audit F5 / W1-audit D-2; Doc 00 §11.65).** The clause **"`InterpreterSnapshot` (`active_states` + `history` + `defer_set` + `context` + `virtual_clock_ms` — already `Clone`, already the complete state)"** above is **an overstatement for the hierarchical/timer scope** and was corrected in the shipped code. The pre-W2 `InterpreterSnapshot` was **lossy** for `RuntimeState.submachines` (the live, recursively-nested sub-instance configs) and `RuntimeState.timers` (the armed timer set) — it did **not** capture them. The post-W1 §11.3 audit (`docs/AUDIT_PHASE_V1_4_W1_2026_05_16.md` D-2 / §1.3) flagged this as the load-bearing W2-blocker: driving the explorer through `restore(snapshot)` over a submachine/timer config would silently drop the nested sub-instances + timers, the digest would conflate behaviourally-distinct configurations, the visited-set would prune prematurely, and the explorer could return a **false `ProvenNoDeadlock`** (the cardinal verification sin). W1 was *correctly fenced* (it loud-rejected composite/parallel/history/timer/submachine via `reject_non_flat`, so W1 shipped sound). **W2's first unit (`efae559`, "Make InterpreterSnapshot lossless for timers + submachines (W2 P0)") extended the snapshot losslessly** — recursive `submachines` + a sorted `timers` `Vec`, **purely additive** (pre-W2 fields keep name/type/order; flat snapshots serialise byte-identically with `"timers":[]`/`"submachines":{}`) — gated by a real ~435-line `snapshot → perturb → restore → re-snapshot` byte-identity round-trip test (`crates/fsm-simulator/tests/snapshot_lossless.rs`). The post-W2 §11.3 audit (`docs/AUDIT_PHASE_V1_4_W2_2026_05_16.md` Lens 3 / F4) independently confirmed the closure. So: the sentence above describes the *flat-machine* state; for the hierarchical/timer scope this wave targets, the complete configuration also includes `submachines` (recursive) + `timers`, made lossless by W2-P0. The original prose is preserved as the wave-plan-of-record; this note is the correction.
...
> ⚠ **reconciliation note (v1.4 closeout — carry-list item 6 / W2-audit F7; Doc 00 §11.66).** The "§11.3 phase-boundary audit after the keystone wave (W2)" above describes the *planned* single-audit cadence. The **shipped** §11.3 cadence is **two** audits, re-sequenced by the orchestrator: a **post-W1 keystone audit** (`docs/AUDIT_PHASE_V1_4_W1_2026_05_16.md`, frozen, commit `a75cd69`, verdict **PROCEED-WITH-NOTES**) ran *first* because W1 is the verification-core keystone (the wave that realises the "drive the Interpreter, never fork the semantics" architecture invariant the whole epic stands on, and which shipped materially more than the §4.3 "library-only" sub-brief — auditing the spine *before* W2 builds on it is the stronger application of §11.3, see that doc's "Sequencing note"), **plus** a **post-W2 breadth audit** (`docs/AUDIT_PHASE_V1_4_W2_2026_05_16.md`, frozen, verdict **PROCEED-WITH-NOTES**) at the keystone+breadth boundary before W3. Both are frozen evidence docs (never overwritten). The original prose is preserved; this note records the actual two-audit cadence.
...
> ⚠ **reconciliation note (v1.4 closeout — carry-list item 1 / W1-audit D-1; Doc 00 §11.64).** W1 as shipped (`2866821`, merged `dd2d41e`) delivered **more than the "library-only / flat-only" cut** sketched here and in §4.3. Per the **owner-confirmed authoritative brief** (Doc 00 §11.63 + the `[[feedback_embeded_fsm_pipeline_before_ui]]` gate — the owner's hard bar is a *complete CLI-integratable pipeline*), W1 additionally shipped: the **`fsm verify` CLI subcommand** (the canonical pipeline-grade interface), the **FSM-E0400 / FSM-W0602 emission**, and the **reachability report** — i.e. the surface §4.2-W1/§4.2-W2/§4.3 still scope to W2. This is **authorised, not a deviation**: deferring the CLI to W2 would have left W1 a library with no headless contract to gate against the owner's pipeline-before-UI bar (the v1.2/v1.3 "the authoritative brief evolved past the sub-brief" precedent). The accepted API shape is **`Verdict::Deadlock { witness: Vec<String>, … }`** — a declared-event-name witness, the *better* choice over the §4.3 sketch's `Vec<StepRecord>` (a declared-event sequence is directly replayable and is what the witness semantically *is*; coupling the verifier's public API to the trace-record shape would be gratuitous — and §4.3 itself said "adjust names as the code demands"). The §4.3 "library-only … flat machines / composite·parallel·timer·submachine are W2 — do NOT attempt them" sub-brief is **superseded by the §11.63 owner-confirmed authoritative brief** for the *CLI/diagnostics* scope (W1 still correctly fenced the *explorer* to flat machines via `reject_non_flat`; the hierarchical/timer *exploration* genuinely landed in W2). The original prose is preserved; this note records the W1-shipped reality.
...
> ⚠ **reconciliation note (v1.4 closeout — carry-list item 1+6; Doc 00 §11.64/§11.66).** Two shipped-reality clarifications: (i) the **FSM-E0400 / FSM-W0602 emission landed in W1** (`2866821`), not W2, per the §4.2-W1 note above (W2 *extended* it to the hierarchical/timer reachable set). (ii) The "**flagged FSM-E0400 with a conformance fixture**" acceptance shipped as a **`fsm-verify`-crate acceptance + a real `.fsm` fixture** (`crates/fsm-verify/tests/fixtures/unreachable_island.fsm`, driven through the real parse+analyze+verify pipeline and byte-cross-checked against an independent interpreter BFS — the §5.4 *behavioural*-acceptance bar, never symbol-presence), **not** a formal `tests/conformance/MANIFEST.json` entry. The formal conformance corpus is byte-unchanged at **26 fixtures** (re-derived at this commit; parser 6 + validator 6 + semantic 7 + codegen-c 4 + formatter 3); the E0400/W0602 closure is the *honest implementation* of catalog-reserved-but-unimplemented codes (the §1.3 / R8 drift fixed in the record — the live `DiagnosticCode` count stays **73** because both codes were *catalogued* pre-v1.4; v1.4 gave them their first *emission site*). G7's formal-conformance posture is therefore unchanged — but the verification surface is behaviourally proven by the `fsm-verify` acceptance suites (the §5.4-analogue, the v1.3 Extension-Host-tests precedent for a new subsystem). The original prose is preserved; this note records how the E0400/W0602 acceptance actually ships and that the formal conformance count is unchanged.
...
> ⚠ **reconciliation note (v1.4 closeout — carry-list item 1·iv+1·v; Doc 00 §11.64).** Two shipped-design clarifications, both the same class as the §4.2-W1 surface-choice annotation (the W3 brief explicitly delegated the surface choice; Doc 30 §4.x has known closeout-batched staleness):
>
> 1. **Surface: shipped as a dedicated sibling subcommand `fsm baseline`** (`fc88ec1`), *not* `fsm test --baseline <dir>`. Recorded as the **accepted better choice**: `fsm test` is already a two-mode runner (MANIFEST vs trace-walk) with its **own** exit-code semantics (1 = fixture-fail, 2 = walk-error, 3 = not-a-dir); W3 needs a *distinct* exit-code contract that mirrors `fsm verify`'s 0/1/2/3/4 family with **different** verdict meanings (1 = drift, 2 = inconclusive). A single-purpose subcommand keeps the factory contract clean and is a coherent sibling of `fsm verify` (the reasoning is in `crates/fsm-cli/src/cmd/baseline.rs:25-37`).
> 2. **Baseline-corpus provenance: the SHIPPED design freezes the *current known-good build* capture, NOT an `a036c38` capture.** The committed corpus is `crates/fsm-cli/tests/fixtures/w3_baseline_corpus/` (6 `.fsm.json`: composite/deferred/motor/parallel/submachine/traffic-light), proven **byte-stable** by `committed_corpus_matches_a_fresh_capture_on_this_build` (`crates/fsm-cli/tests/cli_baseline.rs:373-405` — re-records on the current build and asserts byte-identity against the committed corpus). The `a036c38` reference here was the **original-sketch drift-oracle anchor**: `a036c38` *is* the v1.3.0 release commit (`git rev-list -1 v1.3.0` = `a036c38`), so "captured from `a036c38` (the v1.3 semantics)" meant "freeze v1.3's behaviour as the drift oracle." The shipped capture-from-current-known-good design **satisfies that intent** — there is **zero behavioural Rust delta** from v1.3 to the v1.4 corpus (the v1.4 Rust delta is the *additive* `fsm-verify` crate + the W2-P0 lossless-snapshot extension + the verify/baseline CLI; the simulator's flat/legacy behaviour is byte-unregressed, the W2 audit confirmed), so the current-known-good capture **is** v1.3 semantics by construction. The shipped design supersedes the sketched `a036c38`-capture anchor; **no `a036c38` capture provenance is fabricated** — the corpus's provenance is honestly "captured from the current known-good build, byte-stability-gated." Doc 30 §4.2-W3's gate-criterion phrase "the baseline corpus round-trips" is correctly read as **"the committed corpus round-trips clean on the build"** (the shipped `committed_corpus_matches_a_fresh_capture_on_this_build` gate), not "an `a036c38` capture round-trips."
>
> The original prose is preserved; this note records the shipped surface + the honest corpus provenance.
...
> ⚠ **reconciliation note (v1.4 closeout — carry-list item 1; Doc 00 §11.64).** This W1 sub-brief (below) is **superseded for the CLI/diagnostics scope by the §11.63 owner-confirmed authoritative brief** and is preserved here as the wave-plan-of-record only (the v1.2/v1.3 "the authoritative brief evolved past the sub-brief" precedent). Specifically: (a) the brief's `SCOPE BOUNDARY — DO NOT TOUCH … W2 surface: … FSM-E0400/W0602 emission, CLI wiring (`fsm verify` subcommand) — all W2+. W1 is library-only + its acceptance tests.` was **superseded**: W1 *did* ship the `fsm verify` CLI + FSM-E0400/W0602 + the reachability report (`2866821`), authorised by Doc 00 §11.63 + the pipeline-before-UI gate — see the §4.2-W1 note. (W1 still correctly fenced the *explorer* to flat machines via `reject_non_flat`; only the CLI/diagnostics scope expanded.) (b) The brief's `pub enum Verdict { … Deadlock { witness: Vec<StepRecord>, config: Vec<String> }, … }` shipped instead as **`Verdict::Deadlock { witness: Vec<String>, … }`** — the accepted **better** choice (a declared-event-name witness is directly replayable and is what the witness semantically *is*; the brief itself authorised "adjust names as the code demands"). (c) The brief's `#![forbid(unsafe_code)]` count note "(workspace convention, 11 roots/10 crates → becomes 12/11)" is **confirmed correct**: re-derived at this commit, `git grep -l '#![forbid(unsafe_code)]' -- 'crates/*/src/*.rs'` = **12 root files / 11 crates** (the new `crates/fsm-verify/src/lib.rs` is the 12th root; `fsm-lsp` contributes 2 roots lib.rs+main.rs). The original brief prose is preserved unaltered below; this note records what shipped.
