...
> **⚠ v1.3-V1 reconciliation note (Finding 1, corrected-in-shipped — see Doc 00 §11.52; full correction text: `docs/AUDIT_PHASE_V1_2026_05_16.md` §3.A).** The "Decision — stdio `ServerOptions`" paragraph below names **`TransportKind.stdio`**, which is a **wrong literal**: setting `transport: TransportKind.stdio` makes `vscode-languageclient@9` push `--stdio` onto argv for an `Executable`, and the shipped strict server **exits 2 on any unknown arg, including `--stdio`** (`main.rs:49-52`) — crashing the client mid-`initialize`. The shipped V1 client (`6707d1d`) correctly constructs an `Executable` with the `transport` field **deliberately OMITTED** (`{ command, args: [] }`), the *only* form that round-trips (independently re-derived from the locked `vscode-languageclient@9.0.1` source — `main.js:409` reads `json.transport` with no `|| stdio` coercion, `:425` treats `undefined` as a first-class raw spawn). The shipped code is correct; this prose is the stale instruction. **Binding constraint for every `editors/vscode/` wave:** never re-add `transport: TransportKind.stdio` to the `Executable`, never switch to a `NodeModule` server shape. N-3 citation re-pin: the `main.rs:39-48` reference below should read `:39-48` (`--port`) / `:49-52` (unknown-arg exit-2) / `:53-65` (`None → run_stdio`).
...
> **⚠ v1.3-V1 reconciliation note (Finding 2, corrected-in-shipped — see Doc 00 §11.53; full correction text: `docs/AUDIT_PHASE_V1_2026_05_16.md` §3.B).** The claim below that "the entire Doc 26 §4.1 / risk-1 transcoding-defect deletion rides on the server seeing `utf-8` in the offer" rests on a **false premise**: UTF-8 `positionEncoding` is **UNREACHABLE** through the official `vscode-languageclient` (v8/v9 **hardcode** `general.positionEncodings = ['utf-16']` with no override and **throw** on any non-UTF-16 server response — independently re-derived from the locked v9.0.1 `client.js:1370`/`:835-836`). The shipped server's documented **fallback** therefore runs: client offers only `['utf-16']` → server selects **UTF-16** → client accepts. **This is correct** (Doc 26 §4.1: the UTF-16 LineIndex path is still correct — it loses only the *performance* fast-path, not correctness). The substantive multibyte-column defect is guarded **not** by negotiating UTF-8 (impossible) but **end-to-end by the V1 acceptance assertion (d)** under the UTF-16 negotiation, and the shipped acceptance test already asserts `negotiatedPositionEncoding === "utf-16"` (N-2 — the implementer correctly diverged from the stale Doc 28 §3/§5 clause (c) "UTF-8" instruction). The surviving real constraint: do **not** override `clientOptions`/`initializationOptions` to suppress the client's default capabilities (`initializationOptions` carries ONLY the four inlay keys). The §8-V1 acceptance assertion (c) should read **`"utf-16"`**, not "UTF-8". The server-side negotiation code R-4 verified is unchanged and exact; only R-4's *client-reachability assumption* was false.
