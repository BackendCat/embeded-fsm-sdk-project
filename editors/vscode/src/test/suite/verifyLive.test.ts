...
function resultDocFor(base: string): vscode.TextDocument | undefined {
  return vscode.workspace.textDocuments.find(
    (d) =>
      d.uri.scheme === "fsm-verify" &&
      d.uri.query.includes(encodeURIComponent(base)),
  );
}
...
suite(
  "FSM Studio W-C2 (C1-F1+F9) — fsm.verifyLive Extension-Host acceptance",
  () => {
    ...
    suiteSetup(async function () {
      ...
      fs.copyFileSync(
        path.join(EXAMPLES_VERIFY, "clean.fsm"),
        path.join(tmpDir, "clean_d.fsm"),
      );
      ...
      const cleanSrc = fs.readFileSync(
        path.join(EXAMPLES_VERIFY, "clean.fsm"),
        "utf8",
      );
    ...
    test("fsm.verifyLive is registered", async () => {
      ...
      assert.ok(
        all.includes("fsm.verifyLive"),
        "command fsm.verifyLive must be registered",
      );
    });
    ...
    test("(b) clean fixture → the live path shows VERIFIED", async () => {
      ...
      assert.ok(
        ...
        "a verified live result must NOT show INCONCLUSIVE",
      );
    });
    ...
    test("(c) tiny-bound via real fsm/verify LSP request → INCONCLUSIVE, NOT verified", async () => {
      ...
      await client.start();
      try {
        const result = await client.sendRequest<FsmVerifyLiveResult>(
          "fsm/verify",
          { uri: fsmUri, text, maxStates: 1 },
        );
        assert.ok(
          result.verifyJson,
          "the live fsm/verify result must carry a verifyJson envelope",
        );
        ...
        assert.strictEqual(
          v.bound?.hit,
          true,
          "the live A2 result must report the bound as hit",
        );
    ...
    test("(d) debounce coalesces a rapid re-invocation burst", async () => {
      ...
      const matches = vscode.workspace.textDocuments.filter(
        (d) =>
          d.uri.scheme === "fsm-verify" &&
          d.uri.query.includes(encodeURIComponent("clean_d.fsm")),
      );
